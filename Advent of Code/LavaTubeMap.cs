...
namespace Advent_of_Code
{
    public class LavaTubeMap
    {
        ...
        public LavaTubeMap(string[] inputs)
        {
            ...
            {
                Layout.Add(s.Select(c => c.ToString()).Select(int.Parse).ToList());
            }
        }
        ...
        public List<Point> GetLowPoints()
        {
            ...
            int maxX= Layout.First().Count - 1;
            int maxY = Layout.Count - 1;
            ...
            for (int x = 0; x <= maxX; x++)
            {
                for (int y = 0; y <= maxY; y++)
                {
                    ...
                    bool isLowerThanNeighbor = true;

                    if (isLowerThanNeighbor && x != 0)
                    {
                        isLowerThanNeighbor &= height < GetHeightAtPoint(x - 1, y);
                    }

                    if (isLowerThanNeighbor && x != maxX)
                    {
                        isLowerThanNeighbor &= height < GetHeightAtPoint(x + 1, y);
                    }

                    if (isLowerThanNeighbor && y != 0)
                    {
                        isLowerThanNeighbor &= height < GetHeightAtPoint(x, y - 1);
                    }

                    if (isLowerThanNeighbor && y != maxY)
                    {
                        isLowerThanNeighbor &= height < GetHeightAtPoint(x, y + 1);
                    }

                    if (isLowerThanNeighbor)
                    {
        ...
        public int GetProductOfSizeOfThreeLargestBasins(List<Point> lowestPoints)
        {
            ...
            {
                basins.Add(GetPointsInBasin(point));
            }
